...
class UserController extends Controller
{
    ...
    public function aceptDemande(Request $request, $demande_id){



      
            // Vérifier que l'utilisateur est bien connecté en tant qu'entreprise
            if(Auth::user()->role !== 'entreprise') {
                return redirect()->back()->with('error', 'Validé.');
            }
            
            // Trouver la demande d'emploi correspondante dans la base de données
            $demande = DemandeEmploi::find($demande_id);
            
            // Vérifier que la demande existe et qu'elle est liée à l'entreprise actuelle
            if(!$demande || $demande->entreprise_id !== Auth::user()->id) {
                return redirect()->back()->with('error', 'Validé');
            }
            
            // Accepter la demande d'emploi
            $demande->status = 'acceptée';
            $demande->save();
            
            // Rediriger l'utilisateur vers la page de visualisation des demandes d'emploi
            return redirect()->route('demandes.index')->with('success', 'La demande d\'emploi a été acceptée.');
        }
